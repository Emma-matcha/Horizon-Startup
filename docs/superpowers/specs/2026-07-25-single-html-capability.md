...
- 经本地 HTTP 服务器打开时优先使用内嵌 Vosk 离线识别；直接双击 `file://` 时使用浏览器在线识别，键盘和条件式鼠标仍作为备用。
- 保留“参考测试、非医疗诊断”的明确声明。
...
- 单文件把 42 MB Vosk 模型按需解码为 Blob URL；由于 `file://` 的不透明来源隔离，可靠的离线语音必须从 localhost 打开。麦克风权限在用户点击时先申请，再加载模型。
...
- 若浏览器在 60 秒内无法完成模型初始化，必须自动退出加载态，并继续尝试在线语音或键盘备用。
...
5. 内嵌模型能转换为 Blob URL；localhost 模式可启动 Vosk、打开麦克风并进入本地 listening 状态，`file://` 使用已披露的在线备用。
